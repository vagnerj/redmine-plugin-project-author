...
class ProjectAuthorViewHook < Redmine::Hook::ViewListener
  ...
  def view_projects_form(context ={ })
    ...
    puts "opice zaplatuje view!"
    proj = context[:project]
    form = context[:form]
    puts proj.users.size.to_s
    h = { :label => 'author' }
    puts h.to_s

    puts proj.pretty_print_instance_variables.to_s
    puts proj.attributes.to_s
    puts proj.safe_attribute_names.to_s
    # field, choices, options = {}, html_options = {}
    return 'fooo'
    # form.select :author, proj.users, { :label => 'author' }, {}

    # form.
    # f = ''
    # f << tag.label('author')
    # f << tag.select(:author, options_from_collection_for_select(proj.users, "id", "name") )

    # return tag.p f

    #     form << hidden_field(:rate,:date_in_effect, :value => Date.today.to_s)
    # project = context[:project]
    # c = principals_options_for_select(project.users, project.author)

    # puts "after c"
    # return "fubar"
    # select( :assigned_to_id, principals_options_for_select(@project.users, @project.author), :include_blank => true )
    # return user_collection_for_select_author_options(project)
  end
